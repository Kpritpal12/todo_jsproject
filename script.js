...
function addCard(){
    card_head = document.getElementById("modal-input-box").value;
    createObj(card_head);
    closeModal();
}
function closeModal(){
...
function addList(){
    ...
    done_button.setAttribute('style','font-size:18px;cursor:pointer; height:20px; border-radius:8px;')
...
function createCard(){
    if(arr_of_obj.size==0){
    document.getElementById('outer-container').innerHTML = "NIL";
    first_card = 0;
    }
    else {
...
function completedTask(value){
    ...
    document.getElementById(`${value}`).style.color = '#45114e';
